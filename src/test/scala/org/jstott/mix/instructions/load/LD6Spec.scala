...
class LD6Spec extends FlatSpec with Matchers {

  behavior of "apply"

  it should "create an LD6 instruction when not specifying the field" in {
    val result = LD6.apply(TwoSignedBytes.apply(10))
    ...
    result.C shouldBe MixByte(14)
  }

  it should "create an LD6 instruction when specifying the field" in {
    val result = LD6.apply(TwoSignedBytes.apply(10), Field(2, 3))
    result.A shouldBe TwoSignedBytes(Plus, Seq(MixByte(0), MixByte(10)))
    result.I shouldBe MixByte(0)
  ...
  it should "correctly update the accumulator with field (0:5)" in {
    ...
    val ld1 = LD6(TwoSignedBytes.apply(200))
  ...
  it should "correctly update the accumulator with field (1:4)" in {
    val theWord = Word(Plus, Seq(MixByte(1), MixByte(2), MixByte(3), MixByte(4), MixByte(5)))
    val mix = Mix().withMemoryUpdated(theWord, 200)
    val lda = LD6(TwoSignedBytes.apply(200), Field(1, 4))
    lda.toString shouldBe "LD6 200(1:4)"
